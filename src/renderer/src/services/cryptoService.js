// Browser-compatible Crypto Service using WebCrypto API
// AES-256-GCM encryption compatible with the Electron version
...
class CryptoService {
  ...
  // Derive encryption key from access key (used as both license and encryption seed)
  async deriveKey(accessKey) {
    ...
    this.encryptionKey = await crypto.subtle.deriveKey(
      {
        name: 'PBKDF2',
        salt: encoder.encode('cpm-cloud-salt-2024'),
        iterations: 100000,
        hash: 'SHA-256'
      },
      keyMaterial,
  ...
  async encrypt(data) {
    ...
    // Note: WebCrypto's AES-GCM includes the auth tag in the ciphertext
